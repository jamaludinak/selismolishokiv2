...
@section('title', 'Tambah Jenis Kerusakan')

@section('content')
<div class="container mx-auto p-4 sm:p-6 lg:p-8">
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-3xl font-extrabold text-gray-900">Tambah Jenis Kerusakan Baru</h1>
    ...
    <div class="bg-white p-6 rounded-lg shadow-md">
        <form action="{{ route('jenis_kerusakan.store') }}" method="POST" class="space-y-6">
            @csrf
            <div>
                <label for="nama" class="block text-sm font-medium text-gray-700">Nama Jenis Kerusakan</label>
                <input type="text" name="nama" id="nama"
                       class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-orange-500 focus:border-orange-500 sm:text-sm"
                       placeholder="Masukkan nama jenis kerusakan" value="{{ old('nama') }}" required>
            </div>
            ...
            <div>
                ...
                <input type="number" name="biaya_estimasi" id="biaya_estimasi"
                       class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-orange-500 focus:border-orange-500 sm:text-sm"
                       placeholder="Contoh: 50000 (kosongkan jika belum ada)" value="{{ old('biaya_estimasi') }}" step="0.01">
            </div>
