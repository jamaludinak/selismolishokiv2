...
<div class="container mx-auto p-4 sm:p-6 lg:p-8">
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-3xl font-extrabold text-gray-900">Tambah Ulasan Baru</h1>
        <a href="{{ route('ulasan.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-200 text-gray-700 font-semibold rounded-md shadow-sm hover:bg-gray-300 transition duration-300 ease-in-out">
        ...
        <form action="{{ route('ulasan.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="nama" id="nama"
                       ...
                       placeholder="Masukkan nama pemberi ulasan" value="{{ old('nama') }}" required>
            </div>
            ...
            <div>
                ...
                <textarea name="ulasan" id="ulasan" rows="4"
                          class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-orange-500 focus:border-orange-500 sm:text-sm"
                          placeholder="Tulis ulasan Anda di sini" required>{{ old('ulasan') }}</textarea>
            </div>
            ...
            <div>
                ...
                <input type="number" name="rating" id="rating"
                       class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-orange-500 focus:border-orange-500 sm:text-sm"
                       placeholder="Masukkan rating dari 1 sampai 5" min="1" max="5" value="{{ old('rating') }}" required>
            </div>
            ...
            <div class="flex justify-end mt-6">
                <button type="submit" class="inline-flex items-center px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-orange-600 hover:bg-orange-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 transition duration-300 ease-in-out transform hover:scale-105">
                    <i class="fas fa-star mr-2"></i> Simpan Ulasan
                </button>
            </div>
        </form>
    </div>
