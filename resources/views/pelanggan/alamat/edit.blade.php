...
<body class="bg-gray-100 min-h-screen flex items-center justify-center">
    <div class="w-full max-w-lg bg-white rounded shadow p-8 mt-10">
        <h1 class="text-2xl font-bold mb-6">Edit Alamat</h1>
        <form method="POST" action="{{ route('alamat.update', $alamat->id) }}" class="space-y-4">
            @csrf
            @method('PUT')
            <div>
                <label class="block text-sm font-medium text-gray-700">Alamat <span class="text-red-500">*</span></label>
                <textarea name="alamat" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>{{ $alamat->alamat }}</textarea>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700">Longitude <span class="text-red-500">*</span></label>
                <input type="text" name="longitude" value="{{ $alamat->longitude }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
            ...
            <div>
                ...
                <input type="text" name="latitude" value="{{ $alamat->latitude }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
            ...
            <div>
                ...
                <button type="button" onclick="getLocationFromDevice()" class="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700 mb-2">Ambil Lokasi dari Device</button>
                <small class="text-gray-500 block mb-1">Klik tombol di atas untuk mengisi longitude & latitude otomatis dari GPS HP/PC Anda.</small>
